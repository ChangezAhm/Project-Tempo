...
// Server-side only. Reads the review-question inbox for a template and
// backfills items from the stored understanding on the parser service.
...
export async function GET(
  ...
  try {
    const res = await fetch(`${PARSER_URL}/review/${id}`, {
...
// POST = build (backfill) review items from the stored understanding.
export async function POST(
  ...
  const { id } = await params;
  try {
    const res = await fetch(`${PARSER_URL}/review/${id}/build`, {
      method: "POST",
      headers: PARSER_API_KEY ? { "X-API-Key": PARSER_API_KEY } : {},
    });
    const body = await res.json().catch(() => ({}));
